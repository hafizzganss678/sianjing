...
const isOwner = ownerNumber == sender ? true : ["6285172200670@s.whatsapp.net","6283834558105@s.whatsapp.net"].includes(sender) ? true : false
...
const isDeveloper = ["6285172200670@s.whatsapp.net"].includes(sender) ? true : false
...
thumbnail: fs.readFileSync(setting.pathimg), //Pic
...
const sendContact2 = (jid, numbers, name, quoted, mn) => {
let number = numbers.replace(/[^0-9]/g, '')
const vcard = 'BEGIN:VCARD\n' 
+ 'VERSION:3.0\n' 
+ 'FN:' + name + '\n'
...
"fileName": `Holand•MD${ngazap(prefix)}`,
...
let button_menu = [
{ buttonId: `${prefix}allfitur`, buttonText: { displayText: 'ALL MENU' }, type: 1 },
{ buttonId: `${prefix}owner`, buttonText: { displayText: 'OWNER BOT' }, type: 1 },
{ buttonId: `${prefix}rules`, buttonText: { displayText: 'SYARAT & KETENTUAN PENGGUNA' }, type: 1 }
]
...
case 'allfitur':{
  ...
  let ini_latensii = speed() - ini_timestampp
  let button_menu = [
  { buttonId: `${prefix}owner`, buttonText: { displayText: 'PEMILIK BOT' }, type: 1 }
  ]
  const ini_message_Menu = {
  image: await reSize(setting.pathimg, 300, 200),
  caption: teks_menu2,
...
case 'sewa':
    let sew = `PRICELIST SEWABOT:
HARGA RUNDINGIN AJA SAMA OWN KETIK .owner`
reply(sew)
break
case 'grupwa':
case 'groupwa':
case 'gc_wa':{
reply('https://chat.whatsapp.com/GxrXQtfRfL71Se0Fazen9a')
}
break
case 'listgc': {
...
let vcard = 'BEGIN:VCARD\n'
    ...
    + 'FN:Hafizz.\n'
    + 'ORG:Owner Bot;\n'
    + 'item1.TEL;type=CELL;type=VOICE;waid=6285172200670:+62 851-7220-0670\n' 
    + 'item1.X-ABLabel:Creator Multi device bot\n'
    + 'item2.EMAIL;type=INTERNET: ytbpremium221106@gmail.com\n'
    + 'item2.X-ABLabel:Email\n'
    + 'item3.URL:https://instagram.com/Hafizz_Stylee\n'
    + 'item3.X-ABLabel:Instagram\n'
    + 'item4.ADR:;;Indonesia;;;;\n'
    + 'item4.X-ABLabel:Region\n'
    + 'END:VCARD'
conn.sendMessage(from, { contacts: { displayName: 'Owner Bot', contacts: [{ vcard }] } }, { quoted: msg })
...
let nih_cptiktok = ` *TIKTOK DOWNLOADER BY HOLAND-BOT*
...
const sendMsg = await conn.sendMessage(`${nomor_teman}@s.whatsapp.net`, ini_mess_menfess, { quoted: ftroli2 })
